...
def buscar_libros():
    ...
    try:
        ...
        columns = ['id', 'titulo', 'autor', 'estado']
        libros = [dict(zip(columns, row)) for row in cursor.fetchall()]
        return jsonify(libros)
    finally:
        cursor.close()
...
def buscar_prestamos():
    ...
    try:
        cursor.execute('''
            SELECT p.libro_id, l.titulo, u.nombre as nombre_usuario, 
                   p.fecha_prestamo, u.id as usuario_id
            FROM prestamos p
            JOIN libros l ON p.libro_id = l.id
            JOIN usuarios u ON p.usuario_id = u.id
            WHERE p.estado = 'activo'
        ...
        columns = ['id', 'titulo', 'nombre_usuario', 'fecha_prestamo', 'usuario_id']
        prestamos = [dict(zip(columns, row)) for row in cursor.fetchall()]
        return jsonify(prestamos)
    finally:
        cursor.close()
